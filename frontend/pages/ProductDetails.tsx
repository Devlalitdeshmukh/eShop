...
import { Star, Minus, Plus, ShoppingCart, ArrowLeft, User as UserIcon, Send, Package } from 'lucide-react';
...
const ProductDetails = () => {
  ...
  if (!product) {
    return <div className="p-10 text-center">Product not found <button onClick={() => navigate('/shop')}>Go Back</button></div>;
  }
  ...
  const handleAddToCart = () => {
    ...
    addToast(`Added ${quantity} ${product.name} ${selectedVariant ? `(${selectedVariant.name})` : ''} to cart!`, 'success');
  };
    ...
    <div className="bg-white min-h-screen py-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
          {/* Image */}
          <div className="bg-gray-50 rounded-3xl overflow-hidden shadow-inner flex items-center justify-center p-8">
            <img 
              src={product.image} 
              alt={product.name} 
              className="w-full h-full max-h-[500px] object-contain mix-blend-multiply transition-transform hover:scale-105 duration-500"
            />
          </div>
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-brand-600 font-bold tracking-wider uppercase text-sm">{product.category}</span>
              <span className="text-gray-300">•</span>
              <span className="text-blue-600 font-bold tracking-wider uppercase text-sm">{product.brand}</span>
            </div>
            <h1 className="font-serif text-4xl font-bold text-gray-900 mt-2 mb-4">{product.name}</h1>
            ...
            {product.variants && product.variants.length > 0 && (
              <div className="mb-8">
                <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4">Choose Size / Weight</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              ...
              <button 
                ...
                className="flex-1 bg-gray-900 text-white px-8 py-3 rounded-lg font-bold hover:bg-brand-600 transition-all flex items-center justify-center gap-2 shadow-xl shadow-gray-200 disabled:bg-gray-300 disabled:shadow-none"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    ...
                    {user ? (
                        <form onSubmit={handleSubmitReview} className="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea rows={4} required value={comment} onChange={(e) => setComment(e.target.value)} placeholder="What did you think about this product?" className="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-brand-500 outline-none" />
                            </div>
                            <button type="submit" disabled={submitting} className="w-full bg-brand-600 text-white py-3 rounded-lg font-bold hover:bg-brand-700 transition-colors flex items-center justify-center gap-2">
                        ...
                        <div className="text-center py-8">
                            ...
                            <Link to="/login" className="text-brand-600 font-bold hover:underline">Login Now</Link>
                        </div>
                    )}
                </div>
                ...
                <div className="space-y-6">
                    {reviews.length > 0 ? (
                        ...
                        <div className="text-center py-10 text-gray-500">No reviews yet. Be the first to review this product!</div>
                    )}
                </div>
            </div>
        </div>
