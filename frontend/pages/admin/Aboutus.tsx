...
const Aboutus = () => {
  const { aboutus, refreshAboutus, deleteAboutus } = useStore();
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this section?')) {
      try {
        await deleteAboutus(id.toString());
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500">Manage content for the About Us page.</p>
        </div>
        <Link 
          to="/admin/aboutus/new" 
            ...
            {aboutus.length === 0 ? (
              <tr>
                <td colSpan={3} className="px-6 py-8 text-center text-gray-500">
                  No about us sections found. Add one to get started.
                </td>
              </tr>
            ) : (
              aboutus.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                        <button 
                            onClick={() => navigate(`/admin/aboutus/${item.id}/edit`)}
